...
class Work
  def initialize
    ...
    @hash_of_params =  {1 => "#{@time}", 2 => "#{@subject}",
      3 => "#{@type_of_subject}", 4 => "#{@number_of_subgroup}",
      5 => "#{@lecture_hall}", 6 => "#{@teacher}", 7 =>"#{@note}" }
  ...
  def check
    @counter_of_hash = 1
    if @check_week.to_s.include? "#{@number_of_week}"
      ( "C".."I" ).each do |index_of_column|
        @hash_of_params[@counter_of_hash] = @tablesheet.cell( index_of_column,
        @index_of_row )
        @counter_of_hash += 1
      end
      output
